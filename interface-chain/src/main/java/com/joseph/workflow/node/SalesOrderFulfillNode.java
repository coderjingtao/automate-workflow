 ...
 */
@Slf4j
@SalesOrderNode(NODE_ID = com.joseph.newnetsuite.constant.WorkFlowNodes.SalesOrderNodes.ADD_FULFILL)
public class SalesOrderFulfillNode<T extends Request> implements Node<T, Response> {
    @Override
    public Response process(Chain<T, Response> chain) {
        //处理本节点的任务
        log.info("==================执行Sales Order Fulfill Node开始=========");
        T request = chain.request();
        request.setStatus("SO_FULFILL");
        System.out.println(request.getStatus());
        log.info("==================执行Sales Order Fulfill Node结束=========");

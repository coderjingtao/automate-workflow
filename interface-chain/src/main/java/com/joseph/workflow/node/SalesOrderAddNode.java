 ...
 */
@Slf4j
@SalesOrderNode(NODE_ID = com.joseph.newnetsuite.constant.WorkFlowNodes.SalesOrderNodes.ADD_SO)
public class SalesOrderAddNode<T extends Request> implements Node<T, Response> {

    @Override
    public Response process(Chain<T, Response> chain) {
        //处理本节点的任务
        log.info("==================执行Sales Order Add Node开始=========");
        T request = chain.request();
        request.setStatus("SO_INIT");
        System.out.println(request.getStatus());
        log.info("==================执行Sales Order Add Node结束=========");
